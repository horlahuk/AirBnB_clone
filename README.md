...
The objective is to develop a controlled command line interface that allows us to manage objects within our file storage. Currently, the project focuses solely on building the backend console. It encompasses a comprehensive web application that incorporates both database storage and front-end interaction, resembling a replica of AirBnB.
...
The classes mentioned earlier are managed by the abstract storage engine specified in the [FileStorage].(./models/engine/file_storage.py) class.

Upon initializing the backend, AirBnB creates a new instance of FileStorage referred to as Storage. The storage object is then loaded or reloaded from any class instances present in the JSON file named file.json. Whenever class instances are created, modified, or removed, the storage object is utilized to record the corresponding changes in the file.json file.
